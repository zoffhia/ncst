...
include('functions/admission_functions.php');

$admissionStatusMessage = '';

if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['email'])) {
    $email = $_POST['email'];
    $result = searchStudentByEmail($email);

    if ($result['status'] === 'success') {
        $student = $result['data'];
        $admissionStatusMessage = "Hello " . htmlspecialchars($student['fullName']) . "! Your admission status is: " . htmlspecialchars($student['status']);
    } else {
        $admissionStatusMessage = $result['message'];  // e.g. "No student found..."
    }
}
?>
...
<div class="min-h-screen flex items-center justify-center sm:ml-64 p-4">
  <div class="max-w-md w-full mx-auto bg-white border border-gray-200 rounded-lg shadow-md p-8 z-10">
    ...
    <form method="POST" class="space-y-6">
      <?php if ($admissionStatusMessage): ?>
        <div class="mt-4 p-4 bg-yellow-100 text-yellow-900 rounded border border-yellow-400">
          <?php echo $admissionStatusMessage; ?>
        </div>
      <?php endif; ?>
